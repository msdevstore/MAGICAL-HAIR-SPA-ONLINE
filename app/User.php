...
class User extends Authenticatable
{
    ...
    protected $fillable = [
        'first_name', 'last_name', 'email', 'phone', 'phone_object', 'password', 'gender', 'cellphone', 'avatar', 'point', 'date_of_birth', 'verified', 'role_id','disabled', 'token', 'is_admin', 'notification_check', 'verification_code'
    ];
    ...
    {
        return $this->hasOne(Offdays::class);
    }
    ...
    public static function getUserLists($request)
    {
        ...
        if (!empty($searchValue)) {
            $query = User::select('users.id as id', 'users.first_name', 'users.last_name', 'users.email', 'roles.title as role_title', 'users.is_admin', 'users.disabled')
                ...
                // ->leftJoin('holidays', 'users.id', '=', 'holidays.created_by')
                ->where('title', 'LIKE', '%' . $searchValue . '%')
                ->orWhere('first_name', 'LIKE', '%' . $searchValue . '%')
                ->orWhere('last_name', 'LIKE', '%' . $searchValue . '%')
                ->orWhere('email', 'LIKE', '%' . $searchValue . '%');
        } else {
            $query = User::with(['holidays'])
                ->select('users.id as id', 'users.first_name', 'users.last_name', 'users.email', 'roles.title as role_title', 'users.is_admin', 'users.disabled')
                ->leftJoin('roles', 'users.role_id', '=', 'roles.id')
                // ->leftJoin('holidays', 'users.id', '=', 'holidays.created_by')
    ...
    public static function getAllClients($request)
    {
        ...
        $query =  User::leftJoin('roles', 'users.role_id', '=', 'roles.id')
            ->where('role_id', '=', 0)
            ->where('is_admin', '=', 0)
            ->select('users.id as id', 'users.first_name', 'users.last_name', 'users.email', 'users.phone', 'users.point');
    ...
    public static function getClientBookingList($request, $userId){
        ...
        $query =  User::where('users.id', '=', $userId)
            ...
            ->select('users.first_name', 'users.last_name', 'users.email', 'users.phone', 'services.title', 'bookings.id as id', 'bookings.status', 'bookings.booking_date', 'bookings.booking_time', 'bookings.booking_bill', 'bookings.quantity');
