...
class BookingController extends Controller
{
    ...
    public function invoiceData($column, $id)
    {
        ...
        foreach ($invoiceItemData as $item) {
            ...
            $item->unit_point = $allSet->getCurrency($item->unit_point);
    ...
    public function setBooking(Request $request)
    {
        ...
        $serviceDetails = Services::select('point', 'title', 'service_duration_type', 'business_type', 'percentage', 'auto_confirm')->where('id', $request->id)->first();

        $user_id = User::select('id')->where('phone', $phone)->first();

        $point = $serviceDetails->point;
        ...
        if ($serviceDetails->service_duration_type == 'daily') {
            $paid_amount = $quantity * $point;
        } else {
            if ($serviceDetails->business_type == 'salon') {
                $child_point = $point - (($point * $serviceDetails->percentage) / 100);
                $total_child_point = $child_point * $children;
                $total_adult_point = $point * $adult;
                $paid_amount = ($total_child_point + $total_adult_point) * sizeof($request->slot);
            } else $paid_amount = $quantity * $point * sizeof($request->slot);
        }
        ...
        if ($isAdmin->isAdmin() || $auto_conf == 1) {
            ...
            $client = User::where('id', $user_id->id)->first();
            User::where('id', $user_id->id)->update([
                'point' => ($client->point - $paid_amount)
            ]);
            ...
            if ($booking_id) {
                ...
                $user = Auth::user();
                History::create([
                    'point' => ($user->point - $paid_amount),
                    'bonus' => $paid_amount,
                    'reason' => $user->first_name.' '.$user->last_name.' confirmed for '.$service->title,
                    'user_id' => $user_id->id,
                    'created_by' => $user->id,
                    'created_at' => now(),
                    'updated_at' => now()
            ...
            // InvoiceItem::create(['invoice_id' => $invoiceId, 'service_title' => $title, 'booking_date' => $booking_date, 'booking_time' => $booking_time, 'unit_price' => $point, 'quantity' => $quantity, 'total' => $paid_amount]);
            ...
            // $invoiceStore = Invoice::create(['user_id' => $user_id->id, 'booking_id' => $booking_id->id, 'total' => $paid_amount, 'due' => $paid_amount, 'created_by' => '', 'comment' => $comment]);
            // $invoiceId = $invoiceStore->id;
            // InvoiceItem::create(['invoice_id' => $invoiceId, 'service_title' => $title, 'booking_date' => $booking_date, 'booking_time' => $booking_time, 'unit_point' => $point, 'quantity' => $quantity, 'total' => $paid_amount]);
        ...
        $notifcations->booking_by = User::select('id')->where('phone', $phone)->first()->id;
    ...
    public function action(Request $request, $id)
    {
        ...
        $paid_amount = $status->booking_bill;
        $booking_time = array();
        ...
        $serviceDetails = Services::select('point', 'title', 'service_duration_type')->where('id', $status->service_id)->first();

        $point = $serviceDetails->point;
        $title = $serviceDetails->title;
        $serviceType = $serviceDetails->service_duration_type;

        $user = User::select('id', 'first_name', 'last_name', 'email', 'point')->where('id', $status->user_id)->first();
        ...
        if ($request->status == 'confirmed') {
            ...
            $notifcations->notify_to = $notify;
            $notifcations->save();
            $delta = $user->point - $status->booking_bill;
            if ($delta >= 0){
                User::where('id', $status->user_id)->update(['point' => $delta]);
                Booking::where('id', $id)->update(['status' => 'confirmed']);
                History::create([
                    'point' => $delta,
                    'bonus' => $status->booking_bill,
                    'reason' => 'Confirmed for Service',
                    'user_id' => $user->id,
                ...
                $returnData['message'] = Lang::get('You have not enough points!');
    ...
    public function bookingDetails($id, $return_data = false)
    {
        ...
        $booking = Booking::Join('services', 'bookings.service_id', '=', 'services.id')
            ...
            ->select('bookings.*', 'users.id as user_id', 'users.avatar', 'users.first_name', 'users.last_name', 'users.email', 'users.phone', 'services.title', 'services.point', 'services.service_duration_type')
            ->find($id);

        // $booking['payment_stat'] = $booking->booking_bill - $booking->paid_amount;
        // $booking->paid_amount = $settingAll->getCurrency((string)($settingAll->thousandSep(count(unserialize($booking->booking_time)) * $booking->quantity * $booking->point)));
