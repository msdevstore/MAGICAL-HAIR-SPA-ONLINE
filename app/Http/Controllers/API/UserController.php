...
class UserController extends Controller
{
    ...
    public function resetPassword($id) {
        $result = User::where('id', $id)->update(['password' => bcrypt('123456')]);
        return $result? 1 : 0;
    }

    public function pointManage(Request $request, $id) {
        $user = User::where('id', $id)->first();
        $delta = $user->point + $request->point;
        User::where('id', $id)->update(['point' => $delta]);
        History::create([
            'point' => $delta,
            'bonus' => $request->point,
