                    ...
                    <div class="table-responsive">
                        <table class="table custom-table-responsive">
                            ...
                            <template v-for="column in options.columns">
                                <th :class="{'text-right': column.type=='component' , 'text-right':includes(options.right_align, column.key) }">
                            ...
                            <tr v-for="(data,index) in datasets">
                                <!-- {{data}} -->
                                <template v-for="column in options.columns">
                                    <td :data-label="trans(column.title)"
                                        ...
                                        <!-- Show serially when the column type is with : hasmany -->
                                        <span v-else-if="column.type === 'with'">
                                            <span v-for="value in data[column.key]" v-show="value[column.subkey]">{{value[column.subkey]}}, &nbsp;</span>
                                        </span>

                                        <!-- Show serially when the column type is with : hasmany -->
                                        <span v-else-if="column.type === 'object' && data[column.key]">
                                            <span>{{data[column.key][column.subkey]}}</span>
                                        </span>
                                        ...
                                        <span v-else>
                                            ...
                                            <a :href="publicPath+'/'+column.source+'/'+data[column.uniquefield]"
                                            v-else-if="column.type === 'clickable_link' && column.uniquefield && data[column.uniquefield] && !column.dynamicSource">{{data[column.key]}}</a>

                                            <a :href="publicPath+'/'+data[column.dynamicSource]+'/'+data[column.uniquefield]"
                                            v-else-if="column.type === 'clickable_link' && column.uniquefield && data[column.uniquefield] && column.dynamicSource">{{data[column.key]}}</a>
                                        ...
                                        <span v-if="column.type === 'component' && activeComponent">
                                            ...
                                            <span v-else>
                                                <component v-bind:is="column.componentName" :rowData="data"
                                                        :rowIndex="index" @readData="readData"
                                                        @setPreLoader="setPreLoader"></component>
                                            </span>
                                        </span>
                                        <span v-if="column.type === 'custom_component'">
                                            <span v-if="column.icon">
                                                <component v-bind:is="column.componentName" :icon="column.icon" :rowData="data" :rowIndex="index" :modalId="column.modalId"></component>
                                            </span>
                                            <span v-else-if="column.content">
                                                <component v-bind:is="column.componentName" :content="data[column.content]" :rowData="data" :rowIndex="index" :modalId="column.modalId" :num="column.num"></component>
                                            </span>
                                        </span>
                                    </td>
                                    <!-- For custom input field-->
                                    <td v-if="column.key == options.positionAfter" v-for="(name,index) in columnName">
                                        <span v-if="customDateId.some(function(element) {return element == index &&  data[name] != ''})">
                                            {{datePickerDateFormatter(data[name])}}
                                        </span>
                                        <span v-else>
                                             {{data[name]}}
                                        </span>
                                ...
                                newData.forEach(function (element) {
                                    if (instance.options.formatting) {
                                        if (instance.options.zero_format == true && element.cash_register_opening_amount == 0 && element.cash_register_closing_amount == null) {
                                            if (element.opening_amount == 0) {
                                                ...
                                                element['cash_receives'] = instance.numberFormatSalonGeneralBooking(element.cash_receives);
                                                element['cash_sales'] = instance.numberFormatSalonGeneralBooking(element.cash_sales);
                                            }
                                        }
                                            ...
                                            instance.options.formatting.forEach(function (key) {
                                                element[key] = instance.numberFormatSalonGeneralBooking(element[key]);
                                            });
                                        }
                                    ...
                                    if (instance.options.dateFormatting) {
                                        ...
                                        instance.options.dateFormatting.forEach(function (key) {
                                            if (element[key] !== undefined) element[key] = instance.dateFormatBooking(element[key]);
                                        });
                                    }
